...
A description of the Environment Ontology (ENVO) is published in the Journal of Biomedical Semantics in an article by [Buttigieg et al.](http://dx.doi.org/10.1186/2041-1480-4-43) and a paper describing its development until mid-2016 is available [here](http://dx.doi.org/10.1186/s13326-016-0097-6) 
...
The ENVO repository features an integrated AI-Assisted Curation Agent: **`@edi-ontology-agent`**. This agent is built to assist human curators with repetitive workflow tasks, validation, and schema transformations.
...
* **Metadata & Identifier Verification**: Validates creator ORCIDs, creation dates, synonyms, and identifiers.

### How to Interact
Authorized curators can trigger the agent on any GitHub issue or PR by commenting:
* To create a new ontology term:
> `@edi-ontology-agent please create a new term for <term label> with definition <definition> and parent <parent>`

* To modify or update an existing term:
> `@edi-ontology-agent please update <ENVO:ID> to add <synonym/parent/definition>`

* To map an EDI request:
> `@edi-ontology-agent please map this to an ENVO new term request`

* To request an AI review on a pull request:
> `@edi-ontology-agent please review`

For technical guidelines, see `AGENTS.md`.
